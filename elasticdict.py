...
class ElasticDict(dict):
    """
    A dictionary that automatically dumps it's keys/values
    in Elastic Search. Everything is converted to JSON behind 
    the scenes, then sent over the wire into ES.
    """
    ...
    def __init__(self, es_host='localhost', es_port=9200, es_index='elasticdict', es_type='data'):
        ...
        # Just create an ES index. ES returns a 400 if the index already exists.
        index_data = {"host":self.es_host, "port":self.es_port, "index":self.es_index}
        response = requests.put("http://{host}:{port}/{index}".format(**index_data))
    ...
    def __setitem__(self, key, value):
        ...
        # We need to build a dict to convert to JSON, which then 
        # gets shipped to elasticsearch
        data = {key: value}
